...
ABell::ABell()
{
	...
	capsule->OnComponentBeginOverlap.AddDynamic(this, &ABell::player_overlap);
...
void ABell::ResetAudioComponent()
{
    if (AudioComponent)
    {
        // 오디오 컴포넌트를 새로 초기화
...
void ABell::player_overlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
    ...
    if (sp != nullptr)
    {
        ...
        // 타이머로 4초 후 맵 전환
        GetWorld()->GetTimerManager().SetTimer(TimerHandle, TimerCallback, 3.0f, false);  // 4초로 설정
